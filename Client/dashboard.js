  ...
  div.innerHTML = `
    <div class="bg-white bg-opacity-50 backdrop-blur-lg p-6 rounded-lg shadow-lg col-span-1 md:col-span-3 hover:shadow-xl transition-all">
      <div class="flex justify-between items-center mb-4">
        <h3 class="text-xl font-semibold text-blue-800">${task.name}</h3>
        <span class="text-sm text-purple-500 ml-4">${task.dueDate}</span>
      </div>
      <p class="text-gray-700 mb-4">${task.description}</p>
      <div class="flex items-center mb-4">
        <span class="task-status px-3 py-1 text-sm font-medium text-gray-800 bg-gray-100 rounded-full">
          ${task.status}
        </span>
      </div>
